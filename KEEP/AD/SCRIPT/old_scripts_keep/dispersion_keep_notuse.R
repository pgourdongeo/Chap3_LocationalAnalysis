...
#### test en filtrant les points superposés #############
partners_inEU <- partners_inEU %>%
  group_by(Period) %>% 
...
MeanDistNN(sf = partners_inEU %>% filter(Period == "2000-2006"), k= 1) # = 1855 m / 12371 with unique gnId
...
#### display and save
pdf(file = "AD/OUT/indiceR_U.pdf", width = 8.3, height = 5.8)
Rindex
dev.off()  
...
truc <- ggplot() +
  ...
  geom_point(data = dfPartners %>% filter(Period == "2014-2020"), 
             aes(x = x, y = y),
...
#### display and save
#pdf(file = "AD/OUT/.pdf", width = 8.3, height = 5.8)
gravity
dev.off() 
